 ...
 * Added Sprinter department support for Mercedes-Benz dealerships
 * - Separate Sprinter-Footer snippet support
 * - UI enhancement to select snippet type (Footer or Sprinter-Footer) for MB dealerships
 * - 11 new Sprinter-specific columns in Google Sheets
 *
 * @version 1.0.0 - 2024-10-10
 * Initial implementation with single and multi-dealership footer support
 */
...
/**
 * Checks if a workspace is a Mercedes-Benz dealership.
 * @param {string} workspaceName The workspace name to check.
 * @returns {boolean} True if the workspace is a Mercedes-Benz dealership.
 */
function isMercedesBenzWorkspace(workspaceName) {
  return workspaceName.includes('MB_') || workspaceName.includes('MERCEDES_BENZ');
}
...
</p>${dealershipName === 'Land Rover Redwood City' ? '<p style="margin:10px 0 0 0;text-align:justify;line-height:150%;color:#ffffff">*All Pre-Owned Vehicle Pricing and Advertised Prices for any/all vehicles offered for sale do not include dealer installed accessories which can be purchased for an additional cost and are optional. Dealer Installed accessories include DOWC Max Shield $1,995.</p>' : ''}
</td>
</tr>
{% endif %}
<tr>
<td align="center" style="background-color:#000000;padding:10px 20px;font-family:Arial,sans-serif;font-size:10px;line-height:1.5;color:#ffffff" class="mobile-padding">
<p style="margin:0;text-align:center;line-height:150%;color:#ffffff">
Visit <a href="https://swickard.com/privacy-policy/" style="color:#ffffff;text-decoration:underline">https://swickard.com/privacy-policy/</a> to view a copy of our privacy policy and notice of collection.
</p>
</td>
</tr>
<tr>
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (dealership && dealership.trim()) {
      dealershipData[dealership] = {
        ...
        // Sprinter-specific columns for Mercedes-Benz dealerships
        sprinter_footer_phone: getCellValue(row, headers, "sprinter_footer_phone"),
        sprinter_footer_text: getCellValue(row, headers, "sprinter_footer_text"),
        sprinter_new_inventory_footer_url: getCellValue(row, headers, "sprinter_new_inventory_footer_url"),
        sprinter_used_inventory_footer_url: getCellValue(row, headers, "sprinter_used_inventory_footer_url"),
        sprinter_service_footer_url: getCellValue(row, headers, "sprinter_service_footer_url"),
        sprinter_gubagoo_footer_url: getCellValue(row, headers, "sprinter_gubagoo_footer_url"),
        sprinter_instagram_footer_url: getCellValue(row, headers, "sprinter_instagram_footer_url"),
        sprinter_facebook_footer_url: getCellValue(row, headers, "sprinter_facebook_footer_url"),
        sprinter_showroom_footer_hours: cleanTextForHTML(getCellValue(row, headers, "sprinter_showroom_footer_hours")),
        sprinter_service_footer_hours: cleanTextForHTML(getCellValue(row, headers, "sprinter_service_footer_hours")),
        sprinter_footer_address: cleanTextForHTML(getCellValue(row, headers, "sprinter_footer_address"))
      };
    }
  }
...
function createLiquidCase(fieldKey, dealerships, allDealershipData, isTel) {
    ...
    // Use two-step assign to check Desired-Make first, then fallback to Vehicle-Make
    let cases = `{%assign m=customer["Desired-Make"]%}{%if m==blank%}{%assign m=customer["Vehicle-Make"]%}{%endif%}{%case m|downcase%}`;
...
function getMultiDealershipTemplate() {
  return `<!--[if mso]><style>.o{width:600px!important;font-size:14px!important}.s{width:32px!important;height:32px!important}</style><![endif]--><!--[if !mso]><!--><style>@media(max-width:600px){.n{display:block!important;padding:10px 0!important}.st{width:100px!important;margin:0 auto!important}.h{width:100%!important;display:block!important;padding:10px 0!important;border-left:none!important}.m{width:100%!important;max-width:100%!important}.p{padding:20px 10px!important}}u~div .g{background-color:#000!important}</style><!--<![endif]--><div><!--[if mso]><table border=0 cellpadding=0 cellspacing=0 width=600 align=center class=o><![endif]--><!--[if !mso]><!--><table border=0 cellpadding=0 cellspacing=0 width=100% style="max-width:600px;margin:0 auto;font-family:Arial,sans-serif" class=m><!--<![endif]--><tr><td align=center style="background:#000;padding:30px 20px" class="p g"><!--[if mso]><table border=0 cellpadding=0 cellspacing=0 width=550 align=center><![endif]--><!--[if !mso]><!--><table border=0 cellpadding=0 cellspacing=0 width=100% style=max-width:550px><!--<![endif]--><tr><td style="border-top:1px solid #fff;padding-top:20px"><table border=0 cellpadding=0 cellspacing=0 width=100%><tr><td align=center style=padding-bottom:20px><!--[if mso]><table border=0 cellpadding=0 cellspacing=0><tr><td><a href="{{DYNAMIC_NEW_INVENTORY_URL}}" class=o style="color:#fff;text-decoration:none;font-weight:bold;padding:0 10px">New</a></td><td><a href="{{DYNAMIC_USED_INVENTORY_URL}}" class=o style="color:#fff;text-decoration:none;font-weight:bold;padding:0 10px">Used</a></td><td><a href="{{DYNAMIC_SERVICE_URL}}" class=o style="color:#fff;text-decoration:none;font-weight:bold;padding:0 10px">Service</a></td><td><a href="{{DYNAMIC_GUBAGOO_URL}}" class=o style="color:#fff;text-decoration:none;font-weight:bold;padding:0 10px">Chat</a></td></tr></table><![endif]--><!--[if !mso]><!--><a href="{{DYNAMIC_NEW_INVENTORY_URL}}" class=n style="color:#fff;text-decoration:none;font-weight:bold;font-size:14px;padding:0 10px">New</a><a href="{{DYNAMIC_USED_INVENTORY_URL}}" class=n style="color:#fff;text-decoration:none;font-weight:bold;font-size:14px;padding:0 10px">Used</a><a href="{{DYNAMIC_SERVICE_URL}}" class=n style="color:#fff;text-decoration:none;font-weight:bold;font-size:14px;padding:0 10px">Service</a><a href="{{DYNAMIC_GUBAGOO_URL}}" class=n style="color:#fff;text-decoration:none;font-weight:bold;font-size:14px;padding:0 10px">Chat</a><!--<![endif]--></td></tr></table></td></tr><tr><td align=center style=padding-bottom:20px><!--[if mso]><table border=0 cellpadding=0 cellspacing=0 align=center><![endif]--><!--[if !mso]><!--><table border=0 cellpadding=0 cellspacing=0 class=st><!--<![endif]--><tr><td style="padding:0 10px"><a href="{{DYNAMIC_INSTAGRAM_URL}}"><!--[if mso]><img src="https://enfoilk.stripocdn.email/content/assets/img/social-icons/logo-white/instagram-logo-white.png" alt=I width=32 height=32 style=display:block class=s><![endif]--><!--[if !mso]><!--><img src="https://enfoilk.stripocdn.email/content/assets/img/social-icons/logo-white/instagram-logo-white.png" alt=I width=32 height=32 style=display:block><!--<![endif]--></a></td><td style="padding:0 10px"><a href="{{DYNAMIC_FACEBOOK_URL}}"><!--[if mso]><img src="https://enfoilk.stripocdn.email/content/assets/img/social-icons/logo-white/facebook-logo-white.png" alt=F width=32 height=32 style=display:block class=s><![endif]--><!--[if !mso]><!--><img src="https://enfoilk.stripocdn.email/content/assets/img/social-icons/logo-white/facebook-logo-white.png" alt=F width=32 height=32 style=display:block><!--<![endif]--></a></td></tr></table></td></tr><tr><td><!--[if mso]><table border=0 cellpadding=0 cellspacing=0 width=100%><tr><td width=275 valign=top style="font-family:Arial,sans-serif;font-size:12px;line-height:1.3;color:#fff;padding-right:15px"><p style="margin:0 0 10px 0;text-align:center"><b>Showroom Hours:</b></p><p style="margin:0 0 5px 0;text-align:center;line-height:1.3">{{DYNAMIC_SHOWROOM_HOURS}}</p></td><td width=275 valign=top style="font-family:Arial,sans-serif;font-size:12px;line-height:1.3;color:#fff;padding-left:15px;border-left:1px solid #333"><p style="margin:0 0 10px 0;text-align:center"><b>Service Hours:</b></p><p style="margin:0 0 5px 0;text-align:center;line-height:1.3">{{DYNAMIC_SERVICE_HOURS}}</p></td></tr></table><![endif]--><!--[if !mso]><!--><table border=0 cellpadding=0 cellspacing=0 width=100%><tr><td width=50% valign=top style="font-family:Arial,sans-serif;font-size:12px;line-height:1.3;color:#fff;padding-right:15px" class=h><p style="margin:0 0 10px 0;text-align:center"><b>Showroom Hours:</b></p><p style="margin:0 0 5px 0;text-align:center;line-height:1.3">{{DYNAMIC_SHOWROOM_HOURS}}</p></td><td width=50% valign=top style="font-family:Arial,sans-serif;font-size:12px;line-height:1.3;color:#fff;padding-left:15px;border-left:1px solid #333" class=h><p style="margin:0 0 10px 0;text-align:center"><b>Service Hours:</b></p><p style="margin:0 0 5px 0;text-align:center;line-height:1.3">{{DYNAMIC_SERVICE_HOURS}}</p></td></tr></table><!--<![endif]--></td></tr><tr><td align=center style=padding-top:20px><p style="margin:0 0 5px 0;font-family:Arial,sans-serif;font-size:16px;color:#fff;font-weight:bold">{{DYNAMIC_DEALERSHIP_NAME}}</p><p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#fff">{{DYNAMIC_ADDRESS}}</p></td></tr><tr><td align=center style=padding-top:10px><p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#fff"><a href="tel:{{DYNAMIC_PHONE_HREF}}" style="color:#fff;text-decoration:none"><b>{{DYNAMIC_PHONE_DISPLAY}}</b></a></p></td></tr></table></td></tr>
{% if campaign.name == "New_Specials_Journey" or campaign.name == "Sales Specials Journey" %}<tr><td align=center style="background:#000;padding:10px 20px;font-family:Arial,sans-serif;font-size:10px;line-height:1.5;color:#fff" class=p>{% assign v = customer["Sales-Specs-Recs"] %}{% assign d = "" %}{% if v[0].Hero_Disclaimer != blank and v[0].Hero_Disclaimer != nil %}{% assign h = v[0].Hero_Disclaimer %}{% else %}{% assign h = "" %}{% endif %}{% for i in v %}{% capture ds %}<br>{{ i.disclaimer_1 | default: "All vehicles are subject to prior sale and may not be available at the time of inquiry. Pricing does not include applicable taxes, title, licensing, or other fees unless explicitly stated. We strive for accuracy in our listings; however, it is the buyer's responsibility to verify all critical information, including vehicle specifications and availability, before purchase. Financing is subject to credit approval. Please contact our dealership for complete details and terms." }}<br>{% endcapture %}{% assign d = d | append: ds | append: " " %}{% endfor %}<p style="margin:0;text-align:justify;line-height:150%;color:#fff">{% if h != "" %}{{ h }} {% endif %}<br>{{ d | newline_to_br }}</p></td></tr>{% endif %}{% if campaign.name == "Preowned_Specials_Journey" or campaign.name == "Preowned Specials Journey" %}<tr><td align=center style="background:#000;padding:10px 20px;font-family:Arial,sans-serif;font-size:10px;line-height:1.5;color:#fff" class=p>{% assign v = customer["Pre-Owned-Specs-Recs"] %}{% assign d = "" %}{% if v[0].Hero_Disclaimer != blank and v[0].Hero_Disclaimer != nil %}{% assign h = v[0].Hero_Disclaimer %}{% else %}{% assign h = "" %}{% endif %}{% for i in v %}{% capture ds %}<br>{{ i.disclaimer_1 | default: "All vehicles are subject to prior sale and may not be available at the time of inquiry. Pricing does not include applicable taxes, title, licensing, or other fees unless explicitly stated. We strive for accuracy in our listings; however, it is the buyer's responsibility to verify all critical information, including vehicle specifications and availability, before purchase. Financing is subject to credit approval. Please contact our dealership for complete details and terms." }}<br>{% endcapture %}{% assign d = d | append: ds | append: " " %}{% endfor %}<p style="margin:0;text-align:justify;line-height:150%;color:#fff">{% if h != "" %}{{ h }} {% endif %}<br>{{ d | newline_to_br }}</p></td></tr>{% endif %}{% if campaign.name contains "Pre_Owned" or campaign.name contains "Pre-Owned" or campaign.subtype == "pre_owned" or campaign.subtype == "pre-owned" %}<tr><td align=center style="background:#000;padding:10px 20px;font-family:Arial,sans-serif;font-size:10px;line-height:1.5;color:#fff" class=p><p style="margin:0;text-align:justify;line-height:150%;color:#fff">All vehicles are one of each and subject to prior sale. All Pre-Owned or Certified vehicles are used. All offers expire at the close of business on the day the offer is removed from the website, and all financing is subject to credit approval. Prices exclude taxes, title, registration, tags, labor and installation charges, insurance, optional equipment, products, packages and services.  Although we attempt to include the cost of all options preinstalled by the dealer in the listed price, some options may have been added that are not reflected in the sales price on the website.  While great effort is made to ensure the accuracy of the information on the website, errors do occur so please verify information with one of our dealership representatives.  Options, model availability and actual dealer price may vary. See dealer for details, costs and terms. Dealer sets final price.</p></td></tr>{% endif %}<tr><td align=center style="background:#000;padding:10px 20px;font-family:Arial,sans-serif;font-size:10px;line-height:1.5;color:#fff" class=p><p style="margin:0;text-align:center;line-height:150%;color:#fff">Visit <a href="https://swickard.com/privacy-policy/" style="color:#fff;text-decoration:underline">https://swickard.com/privacy-policy/</a> to view a copy of our privacy policy and notice of collection.</p></td></tr><tr><td align=center style="background:#000;padding:20px;font-family:Arial,sans-serif;font-size:10px;color:#fff" class=p>Copyright © 2025 {{DYNAMIC_DEALERSHIP_NAME}}, All rights reserved.</td></tr></table><!--[if mso]></table><![endif]--></div>`;
}